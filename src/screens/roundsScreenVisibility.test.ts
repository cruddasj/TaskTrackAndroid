...
describe('shouldShowCategoryGroupingSuggestion', () => {
  it('returns true when there is at least one unfinished task', () => {
    expect(shouldShowCategoryGroupingSuggestion([buildTask({ status: 'done' }), buildTask({ id: 'task-2', status: 'todo' })])).toBe(true);
  });

  it('returns false when all today tasks are finished', () => {
    expect(shouldShowCategoryGroupingSuggestion([buildTask({ status: 'done' }), buildTask({ id: 'task-2', status: 'done' })])).toBe(false);
  });
