...
import { PlanningDayToggle, PlanningDayOption } from '../components/PlanningDayToggle';
...
export const TodaysTasksScreen = () => {
  ...
  const selectedTasks = useMemo(() => state.tasks.filter((task) => task.plannedDate === selectedDateKey), [state.tasks, selectedDateKey]);
  ...
  const saveTask = () => {
    ...
    if (hasDuplicateTodayTaskTitle(state.tasks, selectedDateKey, title, editingTaskId ?? undefined)) {
      setValidationMessage(`A task with this name already exists in ${planningDay === 'today' ? 'today\'s' : 'tomorrow\'s'} tasks.`);
      return;
    }
    ...
    if (editingTaskId) {
      ...
      addTask({ title, description, category, estimateMinutes }, selectedDateKey);
  ...
  return (
    <Stack spacing={2}>
      <Box>
        ...
        <Typography color="text.secondary">Plan tasks for today or tomorrow, then assign same-day tasks into rounds.</Typography>
      ...
      <Dialog open={open} onClose={closeDialog} fullWidth>
        <DialogTitle>{editingTaskId ? `Edit ${selectedDayLabel}'s task` : `Add ${selectedDayLabel}'s task`}</DialogTitle>
        <DialogContent>
          <TextField
            margin="dense"
            label="Task title"
